...
const recentActivities = [
  { id: 1, text: 'Fastify Auth Server rotated session refresh token', time: '10m ago' },
  { id: 2, text: 'NestJS Passport JWT Guard verified user credentials', time: '25m ago' },
  { id: 3, text: 'Task AUTH-102 moved to In Progress by Techno Jerry', time: '1h ago' },
  { id: 4, text: 'Sprint 42 burndown updated (85% completion target)', time: '3h ago' }
];
</script>
...
<template>
  <div className="space-y-8 text-left">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center gap-1">
            <Sparkles className="w-3.5 h-3.5" /> Nuxt 3 Engine Session
          </span>
        ...
        <p className="text-xs sm:text-sm text-slate-300 mt-2 max-w-xl">
          BoardPilot AI Nuxt 3 Client connected to <strong>Fastify (Port 4000)</strong> & <strong>NestJS (Port 3000)</strong> backend engines.
        </p>
      </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="(m, idx) in metrics" :key="idx" className="glass-card p-6 rounded-2xl border border-slate-800 space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-xs text-slate-400 font-medium">{{ m.title }}</span>
          <div :class="['p-2.5 rounded-xl border', m.bg]">
      ...
      <div className="lg:col-span-5">
        <div className="glass-panel p-6 rounded-3xl border border-slate-800">
          ...
          <div className="space-y-3">
            <div v-for="act in recentActivities" :key="act.id" className="p-3 bg-slate-950 rounded-xl border border-slate-800 flex items-start justify-between gap-3 text-xs">
              <div>
