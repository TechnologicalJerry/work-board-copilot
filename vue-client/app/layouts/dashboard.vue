    ...
    >
      <div>
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800">
          <NuxtLink to="/dashboard" className="flex items-center gap-3 overflow-hidden">
            ...
            <span v-if="!collapsed" className="font-bold text-white tracking-tight text-base whitespace-nowrap">
          ...
          >
            <ChevronRight v-if="collapsed" className="w-4 h-4" />
            <ChevronLeft v-else className="w-4 h-4" />
          </button>
        </div>
        ...
        <nav className="p-3 space-y-1.5">
          ...
          >
            <component :is="item.icon" :class="['w-4 h-4 shrink-0', isActive(item.href) ? 'text-white' : 'text-slate-400']" />
            
            <div v-if="!collapsed" className="flex items-center justify-between w-full">
            ...
            <span v-if="collapsed && item.adminOnly" className="w-2 h-2 rounded-full bg-purple-400 absolute top-2 right-2" />
          </NuxtLink>
        </nav>
      </div>
      ...
      <div className="p-3 border-t border-slate-800 space-y-2">
        <div v-if="!collapsed" className="bg-slate-950/80 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
      ...
      <header className="h-16 bg-slate-900/60 border-b border-slate-800 px-4 sm:px-6 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2 bg-slate-950 p-1 rounded-xl border border-slate-800 text-xs">
            ...
            <button
              @click="setRole('TEAM_MEMBER')"
              :class="[
                'px-3 py-1 rounded-lg text-xs font-semibold flex items-center gap-1 transition-all',
                !isAdmin ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'
              ]"
            >
          ...
          <div className="relative">
            ...
            <div v-if="showNotifications" className="absolute right-0 mt-2 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 z-50 text-left">
