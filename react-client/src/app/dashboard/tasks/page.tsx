...
export default function TasksKanbanPage() {
  const [tasks, setTasks] = useState<Task[]>([
    ...
    { id: '3', code: 'REACT-301', title: 'Next.js App Router Dashboard Layout', column: 'in_progress', priority: 'MEDIUM', assignee: 'DEV', storyPoints: 3 },
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-4 rounded-3xl border border-slate-800 space-y-4">
          ...
          <div className="space-y-3">
            {filteredTasks.filter((t) => t.column === 'backlog').map((t) => (
              <div key={t.id} className="p-4 bg-slate-950 rounded-2xl border border-slate-800/80 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded">{t.code}</span>
                  <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${getPriorityBadge(t.priority)}`}>{t.priority}</span>
                </div>
                <p className="text-xs font-medium text-white">{t.title}</p>
                <div className="pt-2 flex items-center justify-between text-[11px] text-slate-400">
                  <span>Points: {t.storyPoints}</span>
                  <button onClick={() => moveTask(t.id, 'in_progress')} className="text-indigo-400 hover:underline">Start →</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="glass-panel p-4 rounded-3xl border border-indigo-500/20 space-y-4">
          ...
          <div className="space-y-3">
            {filteredTasks.filter((t) => t.column === 'in_progress').map((t) => (
              <div key={t.id} className="p-4 bg-slate-950 rounded-2xl border border-indigo-500/30 shadow-md shadow-indigo-500/5 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-mono text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded">{t.code}</span>
                  <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${getPriorityBadge(t.priority)}`}>{t.priority}</span>
                </div>
                ...
                <div className="pt-2 flex items-center justify-between text-[11px] text-slate-400">
                  ...
                  <button onClick={() => moveTask(t.id, 'review')} className="text-purple-400 hover:underline">Review →</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="glass-panel p-4 rounded-3xl border border-purple-500/20 space-y-4">
          ...
          <div className="space-y-3">
            {filteredTasks.filter((t) => t.column === 'review').map((t) => (
              <div key={t.id} className="p-4 bg-slate-950 rounded-2xl border border-purple-500/30 space-y-2">
                <div className="flex items-center justify-between">
                ...
                <div className="pt-2 flex items-center justify-between text-[11px] text-slate-400">
                  <span>Points: {t.storyPoints}</span>
                  <button onClick={() => moveTask(t.id, 'done')} className="text-emerald-400 hover:underline">Approve →</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="glass-panel p-4 rounded-3xl border border-emerald-500/20 space-y-4">
          ...
          <div className="space-y-3">
            {filteredTasks.filter((t) => t.column === 'done').map((t) => (
              <div key={t.id} className="p-4 bg-slate-950 rounded-2xl border border-emerald-500/30 opacity-80 space-y-2">
                <div className="flex items-center justify-between">
            ...
            <form onSubmit={handleAddTask} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Implement OAuth Google Provider"
